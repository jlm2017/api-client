        ...
        if(url) {
          ...
          for (let [routeName, routeDescriptor] of Object.entries(descriptor.itemExtraRoutes)) {
            Object.defineProperty(this, routeName, {
              value: new ExtraRoute(requestCreator, pathJoin(url, routeDescriptor.path))
            });
          }
        }
      }
...
class API {
  constructor(options) {
    ...
    const configuration = 'configuration' in options ? options.configuration : config.resourceConfiguration;
    this.requestCreator = createRequestCreator(options);
    configureResources(this, configuration, this.requestCreator);
  }
}
