...
function httpErrorHandler(err) {
  ...
  if(err.status === 404) {
    ...
    throw new exceptions.BadRequestError('bad request', err.response);
  } else if(err.response.statusType === 4) {
    throw new exceptions.ClientError('unknown client error', err.response);
  } else if(err.response.statusType === 5) {
    throw new exceptions.ServerError('unknown server error', err.response);
  }
...
class RequestCreator {
  ...
  _request(method, path, options) {
    options = options || {};
    let {query, content} = options;

    const url = path.startsWith(this.endpoint) ? buildPath(path) : buildPath(this.endpoint, path);
...
module.exports.pathJoin = buildPath;
